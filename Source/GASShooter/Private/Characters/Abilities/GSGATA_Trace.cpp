...
TArray<FHitResult> AGSGATA_Trace::PerformTrace(AActor* InSourceActor)
{
	...
	if (bUsePersistentHitResults)
	{
		...
		for (int32 i = PersistentHitResults.Num() - 1; i >= 0; i--)
		{
			...
			if (HitResult.bBlockingHit || !HitResult.HitObjectHandle.IsValid() || FVector::DistSquared(TraceStart, HitResult.HitObjectHandle.GetManagingActor()->GetActorLocation()) > (MaxRange * MaxRange))
			{
	...
	for (int32 TraceIndex = 0; TraceIndex < NumberOfTraces; TraceIndex++)
	{
		...
		for (int32 j = TraceHitResults.Num() - 1; j >= 0; j--)
		{
			...
			if (bUsePersistentHitResults)
			{
				...
				if (HitResult.HitObjectHandle.IsValid() && (!HitResult.bBlockingHit || PersistentHitResults.Num() < 1))
				{
					...
					for (int32 k = 0; k < PersistentHitResults.Num(); k++)
					{
						...
						if (PersistentHitResult.HitObjectHandle.GetManagingActor() == HitResult.HitObjectHandle.GetManagingActor())
						{
			...
			else
			{
				...
				if (ReticleIndex < ReticleActors.Num())
				{
					...
					{
						const bool bHitActor = HitResult.HitObjectHandle != nullptr;

						if (bHitActor && !HitResult.bBlockingHit)
						{
							LocalReticleActor->SetActorHiddenInGame(false);

							const FVector ReticleLocation = (bHitActor && LocalReticleActor->bSnapToTargetedActor) ? HitResult.HitObjectHandle.GetManagingActor()->GetActorLocation() : HitResult.Location;
